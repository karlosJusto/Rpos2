// --- HeaderFinal.jsx ---
import React, { useEffect, useState, useContext, useRef } from 'react'; // Añadido useContext
import { db } from '../../firebase/firebase';
import { collection, getDocs } from 'firebase/firestore';
import RelojDistinto from './RelojDistinto';
import PedidoRapido from '../../ordenes/PedidoRapido';
import { Offcanvas, Button, Navbar, Nav, Modal, Form } from 'react-bootstrap';
...
// --- MODIFICACIÓN: Elimina 'libres' de las props ---
const HeaderFinal = ({ mostrarBarra, onDateAccept, currentSelectedDate, showSupervisionHeader }) => {

  // --- MODIFICACIÓN: Obtén 'libres' y otros valores necesarios del contexto ---
  const {
    numeroBarra,
    pedidosConOrigenUno,
    setNumeroBarra,
    libres // <-- Obtén 'libres' directamente del contexto
  } = useContext(dataContext);

  const [show, setShow] = useState(false);

  // --- Estados y funciones para el modal de suma (sin cambios) ---
  ...
  const sumarNumero = (numero) => {
    if (!isNaN(numero)) {
      setNumeroBarra((prevNumero) => (parseFloat(prevNumero) || 0) + numero);
    }
  };
  ...
  // State for the date picker modal (sin cambios)
  const [modalSelectedDate, setModalSelectedDate] = useState(dayjs(currentSelectedDate));
  const [showModal, setShowModal] = useState(false);
  const handleShowDateModal = () => { setModalSelectedDate(dayjs(currentSelectedDate)); setShowModal(true); };
  const handleCloseDateModal = () => setShowModal(false);
  const [isColorChanged, setIsColorChanged] = useState(false);
  const handleDateChangeInModal = (newDate) => { setModalSelectedDate(newDate); };
  const handleAcceptDate = () => { if (onDateAccept) { onDateAccept(modalSelectedDate); } handleCloseDateModal(); };

  const divStyle = showSupervisionHeader ? 'w-[8vw] h-[10vh] bg-[#75adab]' : 'w-[8vw] h-[10vh] bg-[#f2ac02]';
  const theme = createTheme({ palette: { primary: { main: '#f2ac02' } } });
  ...
  // Adjustment functions (sin cambios)
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center w-full p-[0.5vh] mt-[6vh]">
        <div className="grid grid-cols-12 gap-2 w-full fixed top-0 bg-white p-2 z-20">

          {/* ... (Otras columnas sin cambios) ... */}
           {/* Offcanvas Toggle & Online Orders */}
           <div className="flex w-full gap-2" > {/* Removed onClick here if it was only for Offcanvas */}
             <div className="w-1/2 h-[10vh] bg-[#f2ac02] flex justify-center items-center rounded-xl shadow-md cursor-pointer" onClick={toggleOffcanvas}> {/* Added cursor-pointer */}
                {/* ... SVG ... */}
          ...
          {/* --- MODIFICACIÓN: Libres Display ahora usa 'libres' del contexto --- */}
          ...
          <div className={`${divStyle} flex flex-col justify-center items-center rounded-xl shadow-md cursor-pointer`} onClick={handleShowDateModal}>
            <RelojDistinto fecha={currentSelectedDate} />
          </div>
        </div>
      </div>

      {/* ... (Offcanvas y Modales sin cambios) ... */}
       {/* Offcanvas Menu */}
       <Offcanvas show={show} onHide={toggleOffcanvas} placement="start" style={{ width: '120px', top: '123px', background: '#f2ac02', borderTopRightRadius: '30px', borderBottomRightRadius: '30px', zIndex: 1050 }}>
      ...
      <Modal show={showModal} onHide={handleCloseDateModal} size="md" backdrop="static" keyboard={false} centered>
        <Modal.Body>
              ...
              <DemoContainer components={['StaticDatePicker']}>
                <DemoItem>
                  <StaticDatePicker
                    ...
                    sx={{ '& .MuiPickersDay-root': { fontSize: '1.5rem' }, '& .MuiPickersCalendarHeader-root': { fontSize: '1.5rem' }, '& .MuiPickersDay-selected': { backgroundColor: 'blue' }, '& .MuiPickersDay-dayWithMargin': { margin: '2px' } }}
                  />
                </DemoItem>
              </DemoContainer>
